/*
	...
	Windowing and presentation only. The iteration loop and colour palettes
	live in src/core/mandelbrot_cpu_core.cpp so that headless callers get the
	same pixels without dragging in GLFW or OpenGL.
*/
...
private:
    ...
    // Colour palettes now live in src/core/mandelbrot_cpu_core.cpp and are
    // reached through parallelbrot::map_color(), so the window and any
    // headless caller share one implementation.
    ...
    void render() {
        // CPU computation — delegated to the shared core.
        ...
        parallelbrot::render_cpu(view, cpu_buffer.data());
